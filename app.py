...
import yt_dlp  # Use yt-dlp instead of pafy to fetch YouTube video titles
...
# Load the spacy model (required by pyresparser)
try:
...
def fetch_yt_video(link):
    ...
    try:
        ...
        st.error(f"Error fetching YouTube video title: {str(e)}")
        return "Video Title Unavailable"

def embed_youtube_video(video_url):
    """Embed a YouTube video using an iframe."""
    # Convert watch URL to embed URL (e.g., https://www.youtube.com/watch?v=VIDEO_ID to https://www.youtube.com/embed/VIDEO_ID)
...
def run():
    ...
    link = '[©Developed by Mr, Bhumit Tanwar](https://in.linkedin.com/in/bhumit-tanwar)'
    st.sidebar.markdown(link, unsafe_allow_html=True)

    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS PratyushSQL;"""
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                embed_youtube_video(resume_vid)
                ...
                embed_youtube_video(interview_vid)
        ...
        if st.button('Login'):
            if ad_user == 'Bhumit' and ad_password == 'Bhumit123':
                st.success("Welcome Mr , Bhumit Tanwar!")
                # Display Data
